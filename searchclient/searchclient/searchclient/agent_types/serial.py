...
def serial_agent_type(level, initial_state, action_library, goal_description, frontier):

    # Create an action set where all agents can perform all actions
    action_set = [action_library] * level.num_agents

    planning_success, plan = serial_graph_search(initial_state, action_set, goal_description, frontier)
